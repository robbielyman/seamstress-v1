...
pub fn new_texture(width: u16, height: u16) !void {
    var sfba = std.heap.stackFallback(8 * 1024, std.heap.raw_c_allocator);
    const allocator = sfba.get();
...
pub fn new_texture_from_file(filename: [:0]const u8) !void {
...
pub fn add_event() void {
    var ev: c.SDL_Event = std.mem.zeroes(c.SDL_Event);
    ev.type = SDL_USER_EVENT;
    sdl_call(c.SDL_PushEvent(&ev), "screen.post()");
}
