  ...
  if id ~= nil then
    if params.lookup["lfo_" .. id] == nil then
      ...
      params:set_action("lfo_shape_" .. id, function(x)
        self:set("shape", params:lookup_param("lfo_shape_" .. id).options[x])
      end)

      params:add_number("lfo_depth_" .. id, "lfo depth", 0, 100, 0, function(param)
        return (param:get() .. "%")
      end)
      ...
      params:add_number("lfo_offset_" .. id, "lfo offset", -100, 100, 0, function(param)
        return (param:get() .. "%")
      end)
      ...
      params:add_option(
        ...
        "reset lfo to",
        { "floor", "ceiling", "mid: rising", "mid: falling" },
        1
      )
