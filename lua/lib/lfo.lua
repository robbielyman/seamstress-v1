...
-- norns lfo.lua first committed by @dndrks and @sixolet August 2, 2022
...
local function scale_lfo(target)
  if target.baseline == "min" then
    ...
    target.mid =
      util.linlin(target.min, target.max, target.scaled_min, target.scaled_max, (target.min + target.max) / 2)
...
function LFO:add_params(id, separator, group)
  if id ~= nil then
    if params.lookup["lfo_" .. id] == nil then
      ...
      if separator then
        params:add_separator("lfo_sep_" .. separator, separator)
      end
      ...
      params:set_action("lfo_shape_" .. id, function(x)
        self:set("shape", params:lookup_param("lfo_shape_" .. id).formatter(x))
      end)

      params:add_control(
        "lfo_depth_" .. id,
        "lfo depth",
        controlspec.new(0, 100, "lin", 1, 0, nil, 1 / 100),
        function(param)
          return (param:get() .. "%")
        end
      )
      ...
      params:add_control(
        "lfo_phase_" .. id,
        "lfo phase",
        controlspec.new(0, 100, "lin", 1, self:get("phase") * 100, nil, 1 / 100),
        function(param)
          return (param:get() .. "%")
        end
      )
      ...
      params:add_control(
        "lfo_offset_" .. id,
        "lfo offset",
        controlspec.new(-100, 100, "lin", 1, 0, nil, 1 / 200),
        function(param)
          return (param:get() .. "%")
        end
      )
      ...
      params:set_action("lfo_baseline_" .. id, function(x)
        self:set("baseline", string.gsub(params:lookup_param("lfo_baseline_" .. id).formatter(x), "from ", ""))
        _menu.rebuild_params()
      end)

      local mode_options = { "clocked", "free" }
      params:add_option("lfo_mode_" .. id, "lfo mode", mode_options, mode_options[self:get("mode")])
      params:set_action("lfo_mode_" .. id, function(x)
        self:set("mode", params:lookup_param("lfo_mode_" .. id).formatter(x))
        self:set(
          "period",
      ...
      local current_period_as_rate = self:get("mode") == "clocked"
          and lfo_rates[lfo_rates_as_strings[self:get("period")]]
        or lfo_rates[self:get("period")]
      ...
      params:set_action("lfo_clocked_" .. id, function(x)
      ...
      params:set_action("lfo_reset_" .. id, function(x)
        self:reset_phase()
      end)

      local reset_destinations = { "floor", "ceiling", "mid: rising", "mid: falling" }
      params:add_option(
        "lfo_reset_target_" .. id,
        "reset lfo to",
        reset_destinations,
        reset_destinations[self:get("reset_target")]
      )
      params:set_action("lfo_reset_target_" .. id, function(x)
        self:set("reset_target", params:lookup_param("lfo_reset_target_" .. id).formatter(x))
      end)
