...
--[[
  musicutil was written for norns by @markeats
  first committed by @tehn on November 28, 2018
]]
...
function MusicUtil.generate_scale(root_num, scale_type, octaves)
  if type(root_num) ~= "number" or root_num < 0 or root_num > 127 then
    return nil
  end
...
function MusicUtil.chord_types_for_note(note_num, key_root, key_type)
  if type(key_root) ~= "number" or key_root < 0 or key_root > 127 then
    return nil
  end
