...
-- written as norns' lattice.lua
-- @release v2.0
-- @author tyleretters & ezra & zack & rylee
-- norns lattice.lua first committed by @tyleretters January 11, 2021

local Lattice, Sprocket = {}, {}

--- instantiate a new lattice
-- @tparam[opt] table args optional named attributes are:
-- - "auto" (boolean) turn off "auto" pulses from the seamstress clock, defaults to true
...
--- reset the seamstress clock without restarting lattice
...
--- reset the seamstress clock and restart lattice
...
--- use the seamstress clock to pulse
...
function Lattice:new_sprocket(args)
  ...
  args.action = args.action == nil and function(t)
    return
  end or args.action
